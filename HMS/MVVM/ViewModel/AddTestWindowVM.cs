		...
		void ExecuteCreateommand()
		{
			using (DataContext context = new DataContext())
			{
				if ( String.IsNullOrWhiteSpace(TestName) || String.IsNullOrWhiteSpace(Description) )
				{
					if (String.IsNullOrWhiteSpace(TestName) && String.IsNullOrWhiteSpace(Description))
					{
						var messageWindow = new MessageWindow("Please Enter Valid Test Details.\n Make Sure to fill all the fields 😊");
						messageWindow.ShowDialog();
					}
					else if (String.IsNullOrWhiteSpace(TestName))
					{
						var messageWindow = new MessageWindow("Please Enter Valid Name for Test 😊");
						messageWindow.ShowDialog();
					}
					else if (String.IsNullOrWhiteSpace(Description))
					{
						var messageWindow = new MessageWindow("Please Enter Valid Description for Test 😊");
						messageWindow.ShowDialog();
					}
					else
					{
						var messageWindow = new MessageWindow("Please Enter Valid Fee 😊");
						messageWindow.ShowDialog();
					}
				}
				else
