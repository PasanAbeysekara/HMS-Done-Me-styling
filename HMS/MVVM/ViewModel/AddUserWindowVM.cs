		...
		void ExecuteCreateommand()
		{
			using (DataContext context = new DataContext())
			{
				if (String.IsNullOrWhiteSpace(UserName) || String.IsNullOrWhiteSpace(UserPassword))
				{
					if (String.IsNullOrWhiteSpace(UserName) && String.IsNullOrWhiteSpace(UserPassword))
					{
						var messageWindow = new MessageWindow("Please Enter Valid Drug Details.\n Make Sure to fill all the fields 😊");
						messageWindow.ShowDialog();
					}
					else if (String.IsNullOrWhiteSpace(UserName))
					{
						var messageWindow = new MessageWindow("Please Enter Valid Trade User Name 😊");
						messageWindow.ShowDialog();
					}
					else
					{
						var messageWindow = new MessageWindow("Please Enter Valid User Password 😊");
						messageWindow.ShowDialog();
					}
				}
				else
